...
public class CameraBoundingBox : MonoBehaviour
{
    ...
    //const string datasetImagesPath = "D:/City1M_V2_fixed";
    const string datasetImagesPath = "D:/toyDataset_train_fixed";
    ...
    private void Awake()
    {
        ...
        group1TimeSpan = Random.Range(6, 8);
        isUAV = false;
        if (!isUAV)
        {
    ...
    private void Start()
    {
        ...
        if (!Directory.Exists(datasetImagesPath + "/Cam"))
        {
            for (int i = 1; i <= 8; i++)
            {
                Directory.CreateDirectory(datasetImagesPath + "/Cam_" + (i + 5).ToString());
            }
        }
    ...
    void HandleCameraShot()
    {
        if (Time.time > nextShotTime)
        {
            ...
            overlapAreas.Clear();
            bool group1Valid = true;
            // bool group2Valid = true;
    ...
    void CaptureCameraShot()
    {
        ...
        string jpgName = "/Cam_" + (currentCameraIndex + 6).ToString() + "/" + "cam" + (currentCameraIndex + 6).ToString() + "_" + trainTestStatus + "_" + dayNightStr +
    ...
    {
        bool flag1 = limitedPersonCount.Sum() == 0;
        bool flag2 = (tolerance > 100) && (limitedPersonCount.Sum() < 2); // at least capture one
        if (flag1 || flag2)
        {
            currentCameraIndex++;
            tolerance = 0;
    ...
    void SwitchPersons()
    {
        ...
        SetLimitedPersonCount();
        print("groupIndex = " + groupIndex.ToString());
